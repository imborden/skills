# Adversarial Critic — per-task dispatch (jb-plan-pro)

In `jb-plan-pro` the critic is **dispatched fresh per `[adversarial]` task** by the Opus
orchestrator — there is no standing critic and no plan pre-critique (those are the
star→team upgrade in `jb-plan-max`). Default critic agent: `ce-adversarial-reviewer`;
